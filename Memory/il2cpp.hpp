...
namespace il2cpp {
	namespace methods {
		...
		static auto intialize_method = rb::pattern::find(xorstr("GameAssembly.dll"), xorstr("48 83 EC 48 48 8B 05 ? ? ? ? 48 63 90 ? ? ? ?"));
	...
	void init() {
		methods::intialize_method = rb::pattern::find(xorstr("GameAssembly.dll"), xorstr("48 83 EC 48 48 8B 05 ? ? ? ? 48 63 90 ? ? ? ?"));
	...
	uintptr_t init_class(const char* name, const char* name_space = xorstr("")) {
	...
	uintptr_t method(const char* kl, const char* name, int argument_number = -1, char* arg_name = xorstr(""), const char* name_space = xorstr(""), int selected_argument = -1) {
		uintptr_t iter = 0;
		uintptr_t f;
	...
	uintptr_t hook(void* our_func, const char* function_to_hook, const char* class_to_hook, const char* name_space = xorstr(""), int argument_number = -1, char* argument_name = xorstr("")) {
...
uintptr_t mem::hook_virtual_function(const char* classname, const char* function_to_hook, void* our_func, const char* name_space = xorstr("")) {
	uintptr_t search = *reinterpret_cast<uintptr_t*>(il2cpp::method(classname, function_to_hook, -1, xorstr(""), name_space));
